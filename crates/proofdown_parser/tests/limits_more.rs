...
fn input_size_limit_exceeded() {
    ...
    let limits = ParserLimits { max_input_bytes: 512, ..Default::default() };
    let err = parse_with_limits(&big, limits).unwrap_err();
    assert!(matches!(err.kind, proofdown_ast::ErrorKind::LimitExceeded), "expected LimitExceeded, got: {:?}", err);
}
...
fn node_count_limit_exceeded() {
    ...
    for _ in 0..200 { s.push_str("<artifact.json id=\"x\" />\n"); }
    let limits = ParserLimits { max_nodes: 50, ..Default::default() };
