...
fn json_pointer_valid_and_invalid() {
    let ok = comp("artifact.json", &[("id", "x"), ("json_pointer", "/a/b"), ("collapsed", "true"), ("depth", "2")]);
...
fn image_caption_and_bounds() {
    let ok = comp("artifact.image", &[("id", "img"), ("alt", "desc"), ("caption", "An image"), ("max_height", "512")]);
    validate(&doc_with(ok), Some(&Limits::default())).expect("image with caption ok");

    let bad = comp("artifact.image", &[("id", "img"), ("alt", "desc"), ("max_height", "42")]);
    let err = validate(&doc_with(bad), Some(&Limits::default())).unwrap_err();
...
fn table_columns_validation() {
    let ok = comp("artifact.table", &[("id", "t"), ("columns", "/a/b,foo,bar"), ("caption", "Hello"), ("kind", "api")]);
...
fn text_max_lines_bounds() {
    let ok = comp("artifact.text", &[("id", "t"), ("max_lines", "100"), ("caption", "Snippet")]);
